...
import { boolean, check, doublePrecision, index, integer, pgTable, primaryKey, text } from 'drizzle-orm/pg-core';

/**
 * Postgres transcription of schema.sqlite.ts — keep the two files in lockstep
 * (same tables, columns, indexes, constraints). Deliberate mappings:
 * - integer({ mode: 'boolean' })  -> boolean
 * - real                          -> double precision (no float4 rounding)
 * - date-only and timestamp TEXT columns STAY text: 'YYYY-MM-DD' and ISO-8601
 *   UTC strings compare correctly as strings in both dialects, and the app
 *   never wants implicit timezone semantics from pg date/timestamptz.
 * - `(a IS NOT NULL) + (b IS NOT NULL) = 1` -> num_nonnulls(a, b) = 1
 *   (pg can't add booleans).
 */
...
export const petCaretakers = pgTable(
  'pet_caretakers',
  {
    petId: text('pet_id')
      .notNull()
...
export const needs = pgTable(
  ...
  (table) => [
    ...
    check(
      ...
      sql`num_nonnulls(${table.durationValue}, ${table.quantityValue}) = 1`,
    ),
